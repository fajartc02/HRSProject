...
function onDoubleClick(event) {
  ...
  zoomLevel = 10; // Set closer zoom for double-click
...
// _ON
loader.load("assets/ToyotaHRIS39.glb", (gltf) => {
  ...
  const bubble = model.getObjectByName('Sphere');
  ...
  gltf.animations.forEach((clip) => {
    ...
    if (`${clip.name}`.toLowerCase().includes('_on') || `${clip.name}`.toLowerCase().includes('_2on')) {
...
function changeCamera(cameraName, originPosition = false) {
  ...
  let newCamera = cameras[cameraName];
  if (originPosition) {
    // x:  50.30529309888162 y:  12.404995859031795 z:  3.865420323313467
    newCamera.position.x = 50.30529309888162
    newCamera.position.y = 12.404995859031795
    newCamera.position.z = 3.865420323313467
    isFadingOut = true
    detailsComponent()
  }
